...
IUSE="hardened +privacy hwaccel jack iceowl-uxp pulseaudio pgo selinux test +system-icu +system-zlib +system-bz2 +system-hunspell -system-sqlite +system-ffi +system-pixman +system-jpeg +system-libvpx"
RESTRICT="mirror"
...
src_unpack() {
	unpack icedove-uxp-$ID_VER.tar.gz && unpack v$UXP_VER.tar.gz || die "Failed to unpack application source"
	mv "icedove-uxp-$ID_VER" "icedove-uxp" || die "Failed to remove version from application name (broken branding)"
	mv "${WORKDIR}/UXP-${UXP_VER}" "${S}/mozilla"
	cd "${S}"
}
...
src_install() {
    ...
    cd "${S}"
	MOZ_MAKE_FLAGS="${MAKEOPTS}" SHELL="${SHELL:-${EPREFIX%/}/bin/bash}" \
	DESTDIR="${D}" ./mozilla/mach install
